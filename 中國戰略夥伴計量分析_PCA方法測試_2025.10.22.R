...
cat("觀察值:", nrow(panel_CP9623), "| 變數:", ncol(panel_CP9623), "\n")
cat("國家數:", length(unique(panel_CP9623[[country_id_col]])), "\n\n")

# 找出三年都有完整資料的國家
all_vars <- c(economic_factors, sanctional_factors, governance_factors)
common_countries <- NULL

for (year in years) {
  year_data <- panel_CP9623 %>% 
    filter(year == !!year)
  
  # 篩選完整案例
  complete_cases <- complete.cases(year_data[, all_vars])
  year_countries <- unique(year_data[[country_id_col]][complete_cases])
  
  cat(year, "年:", length(year_countries), "國 | ")
  
  common_countries <- if (is.null(common_countries)) {
    year_countries
  } else {
    intersect(common_countries, year_countries)
  }
}

cat("\n✓ 共同國家:", length(common_countries), "國\n\n")

# 篩選資料
panel_common <- panel_CP9623 %>%
  filter(!!sym(country_id_col) %in% common_countries)
...
for (year in years) {
  ...
  year_df <- data.frame(
    country = as.character(result$country_id),
    cluster = result$clusters
  )
  names(year_df)[2] <- paste0("cluster_", year)
  country_tracking <- left_join(country_tracking, year_df, by = "country")
}

cat("國家分群追蹤表 (前15筆):\n")
print(kable(head(country_tracking, 15)))
...
for (year in years) {
  ...
  # 選擇主要變數（排除 n 和應變數）
  main_vars <- c(economic_factors, structural_factors, governance_factors)
