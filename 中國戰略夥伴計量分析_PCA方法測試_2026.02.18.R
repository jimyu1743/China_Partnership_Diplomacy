...
run_qca_engine <- function(data, label, logic_mode, threshold) {
  ...
  qca_data <- data %>% ungroup() %>%
    mutate(
      ...
      # 制度連結邏輯
      FZ_INS = if(logic_mode == "min") {
        pmin(FZ_FTA_FINAL, FZ_ORG_FINAL)
      } else {
        pmax(FZ_FTA_FINAL, FZ_ORG_FINAL)
      },
  ...
  # --- [關鍵修復] 強制轉換為數值 ---
  # 某些版本的 QCA 套件會將 incl 輸出為文字，這裡強制轉回數字
...
if(exists("CP9623_pca")) {
  ...
  cat("################################################################\n")
  run_qca_engine(dt_p1_0212, "思路2-P1 (胡)", "min", 0.65)
  run_qca_engine(dt_p2_1317, "思路2-P2 (習前)", "max", 0.50)
  run_qca_engine(dt_p3_1823, "思路2-P3 (習後)", "max", 0.50)
  
  cat("\n################################################################\n")
  cat("🔬 思路三：漸進過渡模型 (Gradual Adaptation)\n")
  cat("################################################################\n")
  # P1 與思路二相同，略過
  run_qca_engine(dt_p2_1317, "思路3-P2 (習前)", "mean", 0.50)
  # P3 與思路二相同，略過
  
} else {
  stop("請確認 CP9623_pca 是否已載入 R 環境中")
}
