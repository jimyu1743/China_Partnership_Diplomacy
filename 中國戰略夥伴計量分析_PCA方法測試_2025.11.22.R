...
# ============================================================================
# 步驟 2.5：定義 K-means 的最終混合輸入變數集合
# ============================================================================

cat("\n步驟 1: 定義最終 K-means 輸入變數集合 (PC 數量已確定)...\n")

# 1. 🎯 核心 PC 因子 (基於報告決定：經3、制2、治1)
key_pc_cols <- c(
...
# 2. 🎯 額外加入的原始變數 (您指定的地緣政治/外交變數)
original_vars_to_add <- c("FTA", "ORG", "dip_age_std", "dist_std") 

# 3. 組合最終的分群輸入變數集合 (6 個 PC + 4 個原始變數 = 10 個變數)
all_final_hybrid_clustering_vars <- unique(c(key_pc_cols, original_vars_to_add))

cat("✅ K-means 最終輸入變數集合 (總數:", 
    length(all_final_hybrid_clustering_vars), "):\n", 
    paste(all_final_hybrid_clustering_vars, collapse=", "), "\n")
# ============================================================================
# 步驟3: 各年度分群分析
# ============================================================================
...
# 重新定義函數名稱，避免與舊函數混淆，並移除 'level' 參數
cluster_by_year_overall <- function(data, year, pc_cols,
                                    k_range = 2:8, min_n = 5) {
  
  # --- 1. 資料篩選與準備 ---
  # 🎯 修正點 1：移除對 partnership_col (Level) 的篩選
  cat(sprintf("\n【%s年 - 整體結構分析】", year))
  
  subset_data <- data %>%
    filter(year == !!year) %>% # 只保留對年分的篩選
    drop_na(all_of(pc_cols))   # 移除 PC 缺值的國家
  ...
  list(
    year = year,
    level = "Overall", # 🎯 修正點 2：固定 level 標籤為 "Overall"
...
clustering_results_overall <- list()

# 開始迴圈，只依據年份分群
for (year in analysis_years) {
  
  # 建立唯一的 key
  key <- paste0("Y", year, "_Overall")
  
  # 執行函數
  res <- cluster_by_year_overall(
    CP9623_pca, 
    year, 
    # 🎯 關鍵修正：將 'pc_cols' 替換為 'all_final_hybrid_clustering_vars'
    all_final_hybrid_clustering_vars 
  )
  
  # 只有當結果不是 NULL 時才存入 list
  if (!is.null(res)) {
    clustering_results_overall[[key]] <- res
  }
}
# ============================================================================
# 匯出整體分群績效報告 (Overall Clustering Performance Report)
# ============================================================================

cat("\n步驟 4: 匯出整體分群績效報告\n", rep("-", 80), "\n")

# 1. 扁平化 clustering_results_overall 列表，提取關鍵指標
overall_cluster_report <- map_dfr(names(clustering_results_overall), function(key) {
  res <- clustering_results_overall[[key]]
...
# 2. 顯示結果並匯出
cat("\n整體結構分群績效報告 (Overall Cluster Performance Report):\n")
print(overall_cluster_report)

write_csv(overall_cluster_report, "Overall_Cluster_Performance_Report.csv")
cat("\n✅ 報告已儲存: Overall_Cluster_Performance_Report.csv\n")
# ============================================================================
# 步驟4: 多樣化指標計算
# ============================================================================

# 確保 calculate_diversity_metrics 函數已經被定義 (使用您提供的原函數)
# [請確保您的 R 環境中已經載入了您提供的 calculate_diversity_metrics 函數]


# 重新執行計算，但針對整體分群結果 (clustering_results_overall)
diversity_metrics_overall_list <- lapply(clustering_results_overall, calculate_diversity_metrics)

# 將結果轉為 Data Frame 報表
diversity_report_overall <- bind_rows(diversity_metrics_overall_list) %>%
  arrange(year) # 由於 level 都是 "Overall"，只需按年份排序

# 檢查結果
cat("\n✅ 整體結構多樣性指標報告 (Diversity Report Overall):\n")
print(diversity_report_overall)

# 匯出報表
write_csv(diversity_report_overall, "Diversity_Analysis_Report_Overall.csv")
cat("\n✅ 整體多樣性指標報表已產出: Diversity_Analysis_Report_Overall.csv\n")
...
# ============================================================================
# 步驟 5: 彙總分群結果並追蹤國家分群變化 (使用更健壯的提取方法)
# ============================================================================

cat("\n步驟 5: 彙總分群結果並追蹤國家分群變化...\n", rep("=", 80), "\n")

# 假設 country_id_col 已經定義

all_clusters_df <- map_df(clustering_results_overall, function(res) {
  
  # 嘗試提取 country_id, clusters, year
  country_ids <- res$country_id
  clusters <- res$clusters
  year_val <- res$year
  
  # 🚨 關鍵檢查：如果 ID 或 Cluster 結果為空，則跳過此年份。
  if (is.null(country_ids) || is.null(clusters) || length(country_ids) == 0 || length(clusters) == 0) {
    cat(sprintf("[⚠️ 警告] 年份 %s (Level %s) 缺乏 ID 或 Cluster 數據 (ID Length: %d, Cluster Length: %d)，跳過。\n", 
                year_val, res$level, length(country_ids), length(clusters)))
    return(NULL)
  }
  
  # 建立數據框
  df <- data.frame(
    country_id_placeholder = country_ids,
    year = year_val,
    cluster_group = clusters,
    stringsAsFactors = FALSE
  )
  
  # 使用 setNames 動態命名國家 ID 欄位
  names(df)[1] <- country_id_col 
  
  return(df)
})

# 2. 進行分群變化追蹤 (以下使用 all_clusters_df 進行連接，無需修改)

# --- A. 比較 1996 年到 2013 年的變化 ---
clusters_96_13 <- all_clusters_df %>%
  filter(year %in% c(1996, 2013)) %>%
  pivot_wider(
    id_cols = all_of(country_id_col), 
    names_from = year, 
    values_from = cluster_group,
    names_prefix = "Cluster_"
  ) %>%
  mutate(
    Change_96_13 = Cluster_1996 != Cluster_2013
  )

# --- B. 比較 2013 年到 2023 年的變化 ---
clusters_13_23 <- all_clusters_df %>%
  ...
  mutate(
    Change_13_23 = Cluster_2013 != Cluster_2023
  )

# 3. 合併所有變化追蹤結果
cluster_change_tracking <- full_join(
  clusters_96_13, 
  clusters_13_23, 
  by = all_of(country_id_col)
) %>%
  select(
    all_of(country_id_col), 
    starts_with("Cluster_1996"),
    starts_with("Cluster_2013"),
    starts_with("Cluster_2023"),
    starts_with("Change_")
  )

# 4. 顯示並匯出結果
cat("\n✅ 分群變化追蹤報告 (分群標籤不代表絕對群組，需結合中心點解釋):\n")
print(cluster_change_tracking %>% filter(Change_96_13 == TRUE | Change_13_23 == TRUE))

write_csv(cluster_change_tracking, "Cluster_Change_Tracking_Report.csv")
cat("\n✅ 報告已儲存: Cluster_Change_Tracking_Report.csv\n")
# ============================================================================
# 生成CP9623_final
# ============================================================================
